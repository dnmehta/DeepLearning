...
def mlp_mnist(learning_rate=0.01, n_epochs=250,
                           dataset='symmetry_data.pkl.gz',
                           batch_size=10,
                           n_hidden_1=20,n_hidden_2=5, L1_reg=0.00,L2_sq=0.001,
                           ):
        ...
        for minibatch_index in range(n_train_batches):
            ...
            if (iter + 1) % validation_frequency == 0:
                ...
                if this_validation_loss < best_validation_loss:
                    ...
                    with open('symmetry_mlp_best_model.pkl', 'wb') as f:
...
def predict():
    ...
    classifier = pickle.load(open('symmetry_mlp_best_model.pkl'))
    ...
    probabilities,y_pred = predict_model(test_set_x[0:200])
    print(test_set_y.eval()) #without bias
    for i in range(0,200):

        if probabilities[i][0] > 0.535:
           y_pred[i]=0
        else:
            y_pred[i]=1
...
# test_set_x,test_set_y=datasets[2];
# test_set_x=test_set_x.get_value();
# predicted_values = predict_model(test_set_x[0:201])
